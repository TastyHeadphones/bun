...
describe("hoisted", () => {
  ...
  test.concurrent("a plain install removes an unlisted workspace at an absolute path in bun.lock", async () => {
    const { packageDir } = await relocatedOtherTree(absoluteOtherPath, Object.keys(monorepo.packages));
    const { full: expected } = await fullInstall("hoisted", {
  ...
  test.concurrent("a workspace path in bun.lock that does not fit a path buffer is treated as pruned", async () => {
    const longPath = Buffer.alloc(100_000, "a").toString();
    const { packageDir, lock } = await relocatedOtherTree(() => longPath, survivors);
