...
impl<R> StyleRule<R> {
    pub fn to_css(&self, dest: &mut Printer) -> Result<(), PrintErr> {
        ...
        // differ per pass of the parent, so a rule without passes of its own
        // prints in the passes of the closest ancestor that has some.
        ...
        // With nesting compiled away, the nested rules print once, after all
        // passes of this rule. Every variant of a rule then precedes every
        // variant of the rules that follow it in the source, so a browser that
        // accepts only some of the variants still applies the rules in source
        // order.
        if !supports_nesting {
